...
router.post("/upload", (req,res) => {
    ...
    uploader(`http://${process.env.API_URL}/venus/admin/handlefileupload`, formData, req.headers)
    		.then(response => {
...
router.get("/listfiles", (req, res) => {
	...
	listFiles(`http://${process.env.API_URL}/venus/files/listfiles`, req.headers)
	.then(response => {
    	console.log("Response", response);
    	res.send(response);
    })
...
router.get("/fetchcontent", (req, res) => {
	...
	console.log(name)
	fetchcontent(`http://${process.env.API_URL}/venus/files/fetch/`+name, req.headers)
	.then(response => {
    	console.log("Response", response);
    	res.send(response);
    })
